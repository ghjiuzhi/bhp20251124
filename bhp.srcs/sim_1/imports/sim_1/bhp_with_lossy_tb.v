...
//revised:Add QID & Add Multiplier
...
module bhp_with_lossy_tb;
	...
	// [Add Multiplier Step 1] Define wires to connect DUT and the external Multiplier
	wire [255:0] w_tb_mul_a;          // Carries operand A from DUT to Multiplier
    wire [255:0] w_tb_mul_b;          // Carries operand B from DUT to Multiplier
    wire         w_tb_mul_vld;        // Carries valid signal from DUT to Multiplier
...
/*
  ...
  */ 
    localparam ST_IDLE      = 1;
    localparam ST_INIT      = 2; // Ö®Ç°È±ÉÙÕâ¸ö
    localparam ST_BHP       = 3;
    localparam ST_LOSSY     = 4;
    localparam ST_LOSSY_BUF = 5; // Ö®Ç°È±ÉÙÕâ¸ö
    ...
    // localparam PAD128_0 = 3'b0;
      ...
      reg [WIDTH-1:0] xmem [0:DEPTH-1];// å­˜å‚¨æ•°ç»„ï¼šç´¢å¼? 0..683
      reg [WIDTH-1:0] ymem [0:DEPTH-1];// å­˜å‚¨æ•°ç»„ï¼šç´¢å¼? 0..683
...
// --- [NEW] Lookup Interface Connections ---
		...
		// [Add Multiplier Step 3] Connect to the external Multiplier interface
        // Output ports: Send data from DUT to Testbench Multiplier
    ...
    // [Add Multiplier Step 2] Instantiate the external Multiplier in Testbench
    KOM_Bar_MM #(
        .MUL_WIDTH  ( 256 ) 
    ) u_tb_multiplier (
        .clk        ( clk  ),
        ...
        // Receive requests from DUT
        .i_vld      ( w_tb_mul_vld ), 
        .i_a        ( w_tb_mul_a   ),
        .i_b        ( w_tb_mul_b   ),
        
        // Send results back to DUT
    ...
    initial begin
        ...
        cur_res <= 0;
        zzt_out <= 0;
        reverse <= 0; // 1: è¾“å…¥æ­£åºï¼Œä¸éœ?è¦tbä¸­ååº? 0: è¾“å…¥é€†åºï¼Œéœ€è¦tbä¸­ååº?
        zzt_data <= 0;
        exp_rslt <= 0;
        i_signed <= 0;
        lvs_test <= 0;
...
`else
    $display("Starting bhp_with_lossy pause lvs testbench...");
        reverse  <=  0;
        temp_ready = 1000;
        fd_lvs = $fopen("D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B8_I_lvs.txt", "w"); // "w" è¦†ç›–å†™ï¼›ç”? "a" è¿½åŠ å†?
        // if (fd_lvs == 0) begin
        ...
        $fclose(fd_lvs);
        pathA = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B8_I_lvs.txt";
        pathB = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B8_I_lvs_right.txt";
        check_file_diff(pathA, pathB);

        fd_lvs = $fopen("D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B16_U_lvs.txt", "w"); // "w" è¦†ç›–å†™ï¼›ç”? "a" è¿½åŠ å†?
        lvs_test <=  2;
        zzt_data <=  {PAD16__0,16'b1110110100111010};
        zzt_out  <=  256'd55547;
        #10;zzt_testcase_pause(B16,U,zzt_data,0,zzt_out,temp_ready);
        $fclose(fd_lvs);
        pathA = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B16_U_lvs.txt";
        pathB = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B16_U_lvs_right.txt";
        check_file_diff(pathA, pathB);

        fd_lvs = $fopen("D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B32_U_lvs.txt", "w"); // "w" è¦†ç›–å†™ï¼›ç”? "a" è¿½åŠ å†?
        lvs_test <=  3;
        ...
        $fclose(fd_lvs);
        pathA = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B32_U_lvs.txt";
        pathB = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B32_U_lvs_right.txt";
        check_file_diff(pathA, pathB);

        fd_lvs = $fopen("D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B64_I_lvs.txt", "w"); // "w" è¦†ç›–å†™ï¼›ç”? "a" è¿½åŠ å†?
        lvs_test <=  4;
        ...
        $fclose(fd_lvs);
        pathA = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B64_I_lvs.txt";
        pathB = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B64_I_lvs_right.txt";
        check_file_diff(pathA, pathB);

        fd_lvs = $fopen("D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B128_U_lvs.txt", "w"); // "w" è¦†ç›–å†™ï¼›ç”? "a" è¿½åŠ å†?
        lvs_test <=  5;
        ...
        $fclose(fd_lvs);
        pathA = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B128_U_lvs.txt";
        pathB = "D:\\FPGA\\aleo_prj\\bhp256\\sim_1\\B128_U_lvs_right.txt";
        check_file_diff(pathA, pathB);
        lvs_test <= 0;
`endif
  ...
  initial begin
    ...
    // i_sp_i =  10'd56                                                                           ;// /bhp_with_lossy_tb/uut/uut_bhp_256_top/u_bhp_256/u_montgomery_add_lookup/MG_j çš„æœ€åä¸€ä¸ªæ—¶é’Ÿå‘¨æœŸå¯¹åº”çš„å€¼â†‘
    ...
    // $display("i_sp_i = %0d", ymem[FRAME_i_start-1]);
      ...
      */
      #100;
    // ID
    // xçš?1å€?2å€?3å€?4å€?: xmem[0],xmem[1],xmem[2],xmem[3]
    // yçš?1å€?2å€?3å€?4å€?: ymem[0],ymem[1],ymem[2],ymem[3]
    ...
    while (1) begin
        // RUN_ONCE: begin         // å‘½åå—ï¼šè¢? o_res_vld æ‰“æ–­æ—¶å¯æ•´ä½“é€?å‡?
            ...
            // New Logic: Manually trigger the core with i_start_run.
             #100;
         i_lvs_rdy <= 1;
         i_size    <= B8;  // »ò B64, ¸ù¾İÄúÏë²âµÄÄ£Ê½
        i_signed  <= I;   // Signed   
        i_qid     <= 0;
            #10; 
        //i_vld <= 1; 
        #10;        // Î¬³Ö 1 ¸öÖÜÆÚ (µÈÍ¬ÓÚ @posedge clk)
        //i_vld <= 0;

        // »º³å£º¸ø×´Ì¬»úÒ»µãÊ±¼äÍê³ÉÌø×ª
        #20; 

        // µÚ¶ş²½£º·¢ËÍ Start£¬´¥·¢¼ÆËã (INIT -> BHP)
        #10; 
        //i_start_run <= 1; // Õâ²ÅÊÇÕæÕıµÄµã»ğ£¡
        #10;              // Î¬³Ö 1 ¸öÖÜÆÚ
        //i_start_run <= 0;
            ...
            for (FRAME_i = FRAME_i_start; FRAME_i < 171; FRAME_i = FRAME_i + 1) begin
                // ç­‰å¾… â€œå‘èµ·è¯·æ±‚â?? æˆ? â€œç»“æœæœ‰æ•ˆâ??
                @(posedge top_request or posedge o_res_vld);
                // è‹¥åœ¨ä¸­é?”å‡ºç? o_res_vld ä¸Šå‡æ²¿â?”â?”ç«‹å³é‡å¯æ•´è½? for
                if (o_res_vld) begin
                    // å¯é?‰ï¼šæ‰“ä¸€æ‹ï¼Œé¿å…ç«‹åˆ»å†æ’ä¸ŠåŒä¸?ä¸ªé«˜ç”µå¹³
                    // @(negedge o_res_vld);
                    // disable RUN_ONCE;  // é€?å‡ºå‘½åå—ï¼Œwhile ä¼šç«‹åˆ»å¼€å§‹æ–°ä¸?è½?
                    FRAME_i = 171;
                end
                // æ­£å¸¸æ‰§è¡Œè¿™ä¸€å¸?
                frame_id = FRAME_i;
        ...
        // èµ°åˆ°è¿™é‡Œæœ‰ä¸¤ç§æƒ…å†µï¼š
        // 1) for æ­£å¸¸è·‘å®Œï¼?2) è¢? o_res_vld æ‰“æ–­ disable RUN_ONCEã€?
        // ä¸¤ç§æƒ…å†µä¸‹éƒ½ä¼šç«‹å³å›åˆ? while é¡¶éƒ¨ï¼Œé‡æ–°å¼€å§‹æ–°ä¸?è½? RUN_ONCEã€?
    end
    #10 $finish;
  end
*/
...
// --- [DEBUG 2] ½»»¥ÎÕÊÖ¼à¿Ø ---
    ...
    // --- [DEBUG 3] ³¬Ê±Ç¿ÖÆÍ£Ö¹ ---
    initial begin
        // ÉèÖÃÒ»¸ö×ã¹»³¤µÄÊ±¼ä£¨¸ù¾İ·ÂÕæĞèÒªµ÷Õû£¬ÀıÈç 500us£©
        #500000000; 
        $display("\n--------------------------------------------------");
...
always @(posedge clk) begin
    if (i_lvs_rdy && o_lvs_vld) begin
      // %064hï¼šå›ºå®šå®½åº? 64 ä¸ªåå…­è¿›åˆ¶å­—ç¬¦ï¼Œå‰å¯¼ 0 è¡¥é½ï¼ŒMSB åœ¨å·¦
      if(lvs_test != 0)
...
always @(posedge clk) begin
    // if (i_res_rdy && o_res_vld) begin// res åå‹
    if (o_res_vld) begin// æ²¡æœ‰åå‹
        cur_res <= o_res;
    end
end
...
always @(posedge clk) begin
    if (zzt_testcase_r) begin// æ²¡æœ‰åå‹
...
endtask
  ...
  task output_frame;
    ...
    begin
      ...
      frame_data_out = 32'b0;
      frame_valid_out = 1'b0;
      // éå†FRAMEï¼Œä»æœ?é«?32ä½å¼€å§‹è¾“å‡?
      for (i = (32+WIDTH*8)/32 - 1; i >= 0; i = i - 1) begin
        // ç­‰å¾…æ—¶é’Ÿä¸Šå‡æ²?
        @(posedge clk);
        // è¾“å‡ºå½“å‰32ä½æ•°æ?
        frame_data_out = FRAME[i*32 +: 32];  // ä»ç¬¬i*32ä½å¼€å§‹å–32ä½?
        frame_valid_out = 1'b1;
        // å¯é?‰ï¼šç­‰å¾…ä¸?ä¸ªæ—¶é’Ÿå‘¨æœŸï¼Œä¿æŒè¾“å‡ºç¨³å®š
        // @(posedge clk);
      end
      // æœ?åä¸€ä¸ªæ•°æ®è¾“å‡ºåï¼Œåœ¨ä¸‹ä¸€ä¸ªæ—¶é’Ÿæ²¿å…³é—­æœ‰æ•ˆä¿¡å·
      @(posedge clk);
      frame_valid_out = 1'b0;
      frame_data_out = 32'b0;
      */
  ...
  task xymem_showtest;
    begin
      // ç®?å•éªŒè¯æ‰“å°ï¼šå‰? 2 è¡Œä¸æœ?å? 2 è¡?
      $display("--------------------------------------------------");
  ...
  // ä»»åŠ¡ï¼šä»åè¿›åˆ? TXT é€è¡Œè¯»å…¥åˆ? xmem[]
  task load_xmem_from_decimal_txt;
    begin
      ...
      // ä¸?è¡Œä¸€è¡Œè¯»å–åè¿›åˆ¶åˆ? 256-bit å‘é‡
      for (i = 0; i < DEPTH; i = i + 1) begin
        // %d è¯»å–åè¿›åˆ¶ï¼›\n å…è®¸æŒ‰è¡Œ
